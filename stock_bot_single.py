...
# ══════════════════════════════════════════════
# ضع بياناتك في Render Environment Variables
# ══════════════════════════════════════════════
import os
...
def get_smart_investment_list():
    """جلب أفضل أسهم الاستثمار من Yahoo Finance — يجرب عدة screeners"""
    logger.info("🔍 جلب أفضل أسهم الاستثمار من Yahoo Finance...")
    ...
    for scr_id in screener_ids:
        try:
            ...
            logger.info(f"  ✅ {scr_id}: {len(found)} سهم")
        except Exception as e:
            logger.warning(f"  ⚠️ {scr_id} فشل: {e}")
    
    # إزالة المكرر
    tickers = list(dict.fromkeys(tickers))
    
    # fallback على القائمة الثابتة إذا فشل كل شيء
    if not tickers:
        logger.warning("⚠️ فشل Yahoo screener — استخدام القائمة الاحتياطية")
        tickers = SP500_ALL
...
def get_smart_speculative_list():
    """جلب أفضل أسهم المضاربة من Yahoo Finance — يجرب عدة screeners"""
    logger.info("🔍 جلب أفضل أسهم المضاربة من Yahoo Finance...")
    ...
    for scr_id in screener_ids:
        try:
            ...
            tickers.extend(found)
            logger.info(f"  ✅ {scr_id}: {len(found)} سهم مضاربة")
        except Exception as e:
            logger.warning(f"  ⚠️ {scr_id} فشل: {e}")
...
def fetch_and_analyze(ticker):
    try:
        ...
        logger.warning(f"خطأ في تحليل {ticker}: {e}")
        return None, None, None, None
...
def send_whatsapp(message):
    try:
        ...
        logger.info(f"✅ أرسلت رسالة: {msg.sid}")
        return True
    except Exception as e:
...
def format_signal_message(signal, number):
    ...
    lines += ["", f"*اشتريت {number}؟* رد: *اشتريت {number}*"]
    return "\n".join(lines)
        ...
        try:
            ...
            # اقترب من وقف الخسارة
                ...
                send_whatsapp(
                    f"⚠️ تحذير!\n"
                    f"━━━━━━━━━━━━━━━━━━━\n"
                    f"{num}⃣ *{ticker}* اقترب من وقف الخسارة\n"
                    f"💰 السعر الآن: ${price}\n"
            ...
            # كسر وقف الخسارة
                ...
                send_whatsapp(
                    f"🔴 بيع فوراً!\n"
                    f"━━━━━━━━━━━━━━━━━━━\n"
                    f"{num}⃣ *{ticker}* كسر وقف الخسارة!\n"
                    f"💰 السعر الآن: ${price}\n"
...
def run_server():
    ...
    logger.info(f"🌐 خادم الرسائل يعمل على port {port}")
...
if __name__ == "__main__":
    logger.info("🚀 يعمل بوت الأسهم الذكي v2!")
    ...
    # ملخص صباحي (9:00 بتوقيت نيويورك)
